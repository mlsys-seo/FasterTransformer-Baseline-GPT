...
from examples.pytorch.gpt.utils.ExeGPT.datasets.exegpt_dataset import ExeGPTSimulatedDataset

@torch.no_grad()
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--max_data_cnt', type=int, default=50, metavar='NUMBER',
                        help='Candidate (k) value of top k sampling in decoding. Default is 1.')
    parser.add_argument('--layer_num', type=int, default=24,
                        help='number of layers')
    parser.add_argument('--task', type=str, default="../models/gpt2-vocab.json",
    ...
    dataloader = ExeGPTSimulatedDataset(args.task, return_only_len=True)
    ...
    if rank == 0:
        run_time_list = []
        s_e, e_e = torch.cuda.Event(enable_timing=True), torch.cuda.Event(enable_timing=True)
    while is_finished:
        if rank == 0:
            s_e.record()
        batch_size = max_batch_size
        if args.max_data_cnt < num_data_processed + max_batch_size:
            batch_size = args.max_data_cnt - num_data_processed
            is_finished = False
            
        start_lengths, output_len = dataloader.get_batch(batch_size)
        print(start_lengths)
        print(output_len)
        
        start_ids = [torch.IntTensor([123 for _ in range(b)]) for b in start_lengths]
        start_ids.append(torch.IntTensor([123 for _ in range(int(dataloader.in_seq_max))]))

        start_ids = pad_sequence(start_ids, batch_first=True, padding_value=end_id)[:-1]
        ...
        num_data_processed += batch_size
        if rank == 0:
            e_e.record()
            torch.cuda.synchronize()
            elapsed_time = s_e.elapsed_time(e_e)
            run_time_list.append(elapsed_time)
            print(f"finished: {batch_size}, total: {num_data_processed}, avg_input_seq: {round(int(start_lengths.sum()) / batch_size)}, max_output_seq: {output_len} latency: {round(elapsed_time, 3)}, throughput: {round(batch_size / elapsed_time * 1000, 3)}")
    torch.cuda.synchronize()
    if rank == 0:
        print(f"TOTAL :: elapsed: {round(sum(run_time_list), 3)} throughput: {round(args.max_data_cnt / sum(run_time_list) * 1000, 3)}")
